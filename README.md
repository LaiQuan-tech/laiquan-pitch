...
|---|---|---|
| 1 | 封面 | 巴黎紫 |
| 2 | 這一年（15 專案／8 服務／12 產業）| 巴黎粉 |
| 3 | **服務過的客戶**（14 家具名）| 紫白 |
| 4 | 八種服務（清單）| 白 |
| 5 | 形象官網 ＋ 後台 | 淡紫 |
| 6 | ERP 與營運系統．美強光廣告科技 | 巴黎紫 |
| 7 | ERP 與營運系統．小時光書店 | 綠松石 |
| 8 | **AI CRM**（客戶說過的話，系統都記得）| 白 |
| 9 | 人資與內部管理（好用／穩定／客製化）| 淡紫 |
| 10 | 行動 App | 巴黎紫 |
| 11 | 技術棧（換誰接手都接得下去）| 巴黎粉 |
| 12 | 我們怎麼看 AI（風林火山四 icon 並排）| 深紫 |
| 13–16 | 疾如風／徐如林／侵掠如火／不動如山 | 深紫 |
| 17 | 聯絡我們 | 巴黎紫 |
...
⚠️ **`POS` 底下的標籤必須是 `零售門市`，不要寫成通用的「門市銷售」。**
他們的 POS 是**零售架構**（進貨批次、組合包配貨、庫存簽核、盤點改價），
**目前不支援餐飲**（桌位、併單、出單機、加料都沒有）。
寫成通用的「門市銷售」，餐飲業客戶會要求 demo 一個做不到的系統——
被抓包的代價遠大於少接一個案子。

大字必須維持 `POS` 三個字母：實測在 172px 下，「零售 POS」會折成兩行
（兩個中文字就佔 344px，欄寬只有 356px），另外兩欄是單行，一折整排就歪。
所以限定詞只能放標籤，而且 **`零售` 要放前面**——由上往下讀是 `POS → 零售門市`，
限定詞緊接著 POS 先入眼。

### 第 9 頁：講客戶要的，不要講我們的技術賣點

文案：**「你的流程不用改，系統來配合你。」** 好用／穩定／客製化。

⚠️ **不要寫成「你的系統，掛你的招牌」（白標多租戶）。**
白標的價值是「看不到我們的名字」——那是**賣方自己爽的**，客戶一點都不在乎。
HR 系統的買方真正怕的是三件事：員工不會用、薪資算錯、**套裝軟體逼他改流程**。
最後一項才是客製化真正的賣點（Apollo／MayoHR 這類套裝改不動流程）。

多租戶還是有客戶價值（分店資料各自獨立），但降到註腳，不當標題。

**這跟第 6 頁「講強化不講重做」是同一類錯誤：把技術實作當成客戶需求。**
寫任何一頁之前先問：這句話是在講「我們做了什麼」還是「你會得到什麼」。

### 第 8 頁：AI CRM 講能力，不掛客戶名

文案：**「客戶說過的話，系統都記得。」** 需求變標籤／接手不斷線／主動回訪，
收在「自動化省的是工時，記得客戶省的是回頭客」。

**這頁是餐飲業提案的主力。** 餐飲客戶要的不是 POS，是**客戶關係與需求的標籤建立**——
所以首卡刻意講機制（「每次對話自動拆成偏好標籤，可查也可累積」），不只講結果。
`LaiQuan-tech/dumdum-pitch`（餐飲集團提案）整份**一句 POS 都沒提**，全部圍繞 AI 客戶記憶，
與這個定位一致。

⚠️ **這頁刻意不掛任何客戶名。** nina（美強光，已簽約）的客戶知識庫／報價／回訪／
後台 AI 小幫手在 repo 裡是 **Demo 雛形、全虛構資料**（`is_demo=true` 隔離）。
寫成「我們提供的能力」兩種情況都站得住；一旦掛上客戶名，就變成宣稱那家客戶已經在用。
若日後確認已上真實資料，再改成具名案例。

### 步驟頁的箭頭要在每種底色上都看得見

⚠️ 曾經只寫 `.steps .ar{color:var(--paris-dk)}`，在巴黎紫底（`#7461ad`）上
對比只有 **1.2:1**，整排箭頭等於隱形。現在字級加大到 `clamp(34px,3.4vw,52px)`，
並依底色分色：淺底＝深紫、巴黎紫底＝**白色**（5.18:1）、深紫底＝綠松石。
箭頭是功能性指示（表示流程方向），**清楚優先於品牌色**。

### 第 11 頁：技術棧要講「你會得到什麼」

文案：**「換誰接手，都接得下去。」**
註腳：沒有冷門框架、沒有自研黑箱，台灣找得到人接手維護。

⚠️ **不要寫成「主流、長期維護得下去」。** 那是半個賣方語言——
「主流」「好維護」是我們的技術判斷，客戶聽了沒感覺。
客戶對小型開發商真正的恐懼是：**「你們哪天不做了，我這套怎麼辦？」**
標題要直接回答那個問題。

註腳只主張技術選型（都是主流、沒有自研黑箱），**不要承諾交付原始碼或
無鎖定**——那是合約條款，不是技術事實，簡報寫了會變成沒把握的承諾。
